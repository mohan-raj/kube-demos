...
run "kubectl exec -n tenant-b $STEAL_POD -- \\
	timeout -t 5 redis-cli -h redis.tenant-a -p 6379 SET moneyEarned 0"
...
run "kubectl create -f $(relative tenant-a/redis-policy.yaml)"

desc "Policy for frontend access"
run "cat $(relative tenant-a/frontend-policy.yaml)"
run "kubectl create -f $(relative tenant-a/frontend-policy.yaml)"
